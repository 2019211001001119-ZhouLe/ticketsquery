    ...
    <el-container>
        ...
        <el-main>
            <el-row align="middle" type="flex">
                <el-col :span="16">
                    <el-input v-model="keyword" placeholder="请输入车次关键字"></el-input>
                </el-col>
                <el-col :span="4">
            ...
            <data-tables :data="trainNumbers">
                ...
                <el-table-column>
                    ...
                    <template slot-scope="scope">
                        ...
                        <el-button @click="checkDetails(scope.$index, scope.row)" circle>详情</el-button>
                    </template>
                </el-table-column>
            </data-tables>
        </el-main>
        <el-dialog title="编辑列车" :visible.sync="dialogVisible">
            <el-form :modle="trainNumbers">
                <el-form-item label="车次">
                    <el-input v-model="trainNumbers.routertrainId" :disabled="true"></el-input>
                </el-form-item>
                ...
                <el-form-item label="起始站出发时间">
                    <el-date-picker v-model="trainNumbers.departureTime" type="datetime" placeholder="选择日期时间"
                        align="right">
                    </el-date-picker>
                </el-form-item>
                <el-form-item label="终点站到站时间">
                    <el-date-picker v-model="trainNumbers.arrivalTime" type="datetime" placeholder="选择日期时间"
                        align="right">
                    </el-date-picker>
                </el-form-item>
                <el-form-item>
        ...
        <el-dialog title="添加列车" :visible.sync="addVisible">
            <el-form :modle="newTrainNumbers">
                ...
                <el-form-item>
                    <el-button @click="handleAddSaveClick(newTrainNumber)">保存</el-button>
                </el-form-item>
            </el-form>
        </el-dialog>
    </el-container>
...
export default {
    ...
    methods: {
        ...
        saveTrain(trainNumbers) {
            axios.put(("http://127.0.0.1:8888/train_number/" + trainNumbers["routertrainId"]), trainNumbers).then((response) => {
        ...
        deleteTrain(index, row) {
            console.log(index)
            axios.delete("http://127.0.0.1:8888/train_number/" + row["routertrainId"]).then((response) => {
                console.log(response)
                this.queryAll()
            })
        },
        ...
        addTrain(trainNumbers) {
            axios.post("http://127.0.0.1:8888/train_number", trainNumbers).then((response) => {
                console.log(response)
                this.queryAll()
            })
        },
        ...
        handleBtnClick() {
            axios.get("http://127.0.0.1:8888/train_number/" + this.trainNumbers).then((response) => {
                this.trainNumbers = response.data.data
            })
        },
