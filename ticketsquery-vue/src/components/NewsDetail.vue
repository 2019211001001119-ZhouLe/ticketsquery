<template>
  <div class="home">
        <el-button @click="batchDelete(ids, $event)" style="margin-left: -2px" icon="el-icon-delete-solid"
                   circle></el-button>
        <el-input placeholder="请输入站点名进行搜索，可以直接回车搜索..."
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      // 修改
      // 调用put请求
      axios.put('/details/' + this.detail.routertrainId + '/' + this.detail.trainstationId, this.detail).then(res => {
        console.log(res)
    ...
    onSearch() {
      if (this.keywords) {
        axios.get('/newdetailsbycondition?current=' + this.current + '&size=' + this.size + '&routertrainId=' + this.keywords).then((res) => {
          console.log(res)
          this.details = res.data.records
          this.total = res.data.total
        })
